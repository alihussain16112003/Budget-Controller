...
public class finalbudgetDelete extends JFrame implements ActionListener
{
	Button B;
	JPanel p;
	Choice ch ;
	...
	public finalbudgetDelete()
	{
		...
		public void itemStateChanged(ItemEvent Aobj){
			try{
				...
				Connection Cobj = DriverManager.getConnection("jdbc:mysql://localhost/budget","root"," ");
				Statement sobj = Cobj.createStatement();
				ResultSet rsobj = sobj.executeQuery(" select MemberCount,TotalIncome,IncomeDate,ExpenseType,ExpenseAmount,ExpenseDate,SavingAmount,ShortAmount from finalbudget where ID = '"+ch.getSelectedItem()+"';");
	...
	public void actionPerformed(ActionEvent AObj){
		...
		if(AObj.getSource()==B)
		{
		 ...
		 if(!ch.getSelectedItem().equals("--None--")){			
			...
			else				JOptionPane.showMessageDialog(null,"Plz choose FinalBudgetID");			
		}
	}
	...
	public void del()
	{
		try{
			...
			Connection Cobj = DriverManager.getConnection("jdbc:mysql://localhost/budget","root"," ");
			Statement sobj = Cobj.createStatement();
			String insertQry = "delete from finalbudget where ID=" + ch.getSelectedItem()+";";
